...
class IITree {
	...
	struct Interval {
		S st, en, max_;
		T data;
		Interval(const S& s, const S& e, const T& d) : st(s), en(e), max_(e), data(d) {};
	};
	struct IntervalLess {
	...
	int index_core(std::vector<Interval>& a) {
		...
		for (i = 0; i < a.size(); i += 2) last_i = i, last = a[i].max_ = a[i].en; // leaves (i.e. at level 0)
		for (k = 1; 1LL << k <= a.size(); ++k) { // process internal nodes in the bottom-up order
			...
			for (i = i0; i < a.size(); i += step) { // traverse all nodes at level k
				S el = a[i - x].max_;                          // max value of the left child
				S er = i + x < a.size() ? a[i + x].max_ : last; // of the right child
				S e = a[i].en;
				e = e > el ? e : el;
				e = e > er ? e : er;
				a[i].max_ = e; // set the max value for node i
			}
			last_i = last_i >> k & 1 ? last_i - x : last_i + x; // last_i now points to the parent of the original last_i
			if (last_i < a.size() && a[last_i].max_ > last) // update last accordingly
				last = a[last_i].max_;
		}
		return k - 1;
	}
public:
	...
	void overlap(const S& st, const S& en, std::vector<size_t>& out) const {
		...
		while (t) { // the following guarantees that numbers in out[] are always sorted
			...
			if (z.k <= 3) { // we are in a small subtree; traverse every node in this subtree
				...
				if (y >= a.size() || a[y].max_ > st) // push the left child if y is out of range or may overlap with the query
	...
	bool any_overlap(const S& st, const S& en) const {
		int t = 0;
		StackCell stack[64];
		stack[t++] = StackCell(max_level, (1LL << max_level) - 1, 0); // push the root; this is a top down traversal
		while (t) { // the following guarantees that numbers in out[] are always sorted
			...
			if (z.k <= 3) { // we are in a small subtree; traverse every node in this subtree
				...
				if (st < a[z.x].en) 
					return true; // test if z.x overlaps the query; if yes, append to out[]
				stack[t++] = StackCell(z.k - 1, z.x + (1LL << (z.k - 1)), 0); // push the right child
			}
		}
		return false;
